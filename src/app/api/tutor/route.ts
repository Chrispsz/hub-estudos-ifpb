...
function buildSystemPrompt(
  ...
  return [
    ...
    '- Programa: Álgebra Matricial como NÚCLEO da Av1 (definição/tipos de matriz, soma e escalar, multiplicação e não-comutatividade, transposta, simétrica/antissimétrica, determinante 2×2 = ad−bc, inversa 2×2 = adj(A)/det(A) com teste A·A⁻¹ = I, sistemas 2×2 via AX = B) + Lógica proposicional como revisão contínua (proposições, conectivos, tabelas-verdade, tautologia/contradição, De Morgan, Modus Ponens/Tollens).',
    '- A prova vale 33,3% da média (escala 0-100, aprovação ≥ 70). Peso do estudar BEM matrizes: é o conteúdo central.',
    '- O Hub tem um PLANO DE 12 DIAS (D-12 ao dia da prova) no card do Painel, com tarefas diárias ligadas aos materiais reais (slides Aula 00, lista Aula 01, slides de Lógica 47p, lista de Lógica), 2 SIMULADOS (D-6 e D-1) e fórmulas essenciais. Quando o aluno perguntar o que estudar para a prova, aponte o plano e sugira seguir o dia atual.',
    '- MODO RECUPERAÇÃO (22/09): o aluno entrou no plano com 9 dias restantes — os dias D-11 a D-9 (conceituação, operações, transposta) ficaram para trás e aparecem com selo "atrasado". A recomendação é o catch-up condensado (~90 min: slides Aula 00 + 3 exercícios da Lista 01) ANTES do dia atual, seguindo o card "Plano de Recuperação" do Painel: Matemática (P0) → Algoritmos Semanas 2/3 + Lista Q1–97 (P1) → LM Metadados + Projeto 1ª etapa entrega 09/10 (P2) → Inglês vídeo (P3) → Fundamentos 1 questão/semana (P4) → RHT ADIADO até 02/10 (P5, sem prova marcada).',
    '- Simulado da Prova: botão no card do Painel abre o Simulado Pro já configurado (10 questões de Matemática, 60 min, só conteúdo em sala). As 12 questões novas (mat-ex01 a mat-ex12) vêm EXCLUSIVAMENTE dos materiais reais.',
    '',
